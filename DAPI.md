...
* 第二部分 [上行接口部分](#上行接口部分)
	...
	* 2.64、	[上传声音接口](#上传声音接口)  
	* 2.65、	[保存图片接口](#保存图片接口)  【v2.0新增】
	* 2.66、	[设置是否接收提醒信接口](#设置是否接收提醒信息接口)  【v2.0新增】
	* 2.67、	[举报接口](#举报接口)  
	* 2.68、	[上传视频接口](#上传视频接口)  
...
<h3>1．47、	</h3><h3>统计收藏数量最多接口</h3>  
【参数】  
>  

	usernames:		用户名,多个|连接  
	date:			日期，格式:2014-12-23
	m_auth:			API密钥, 由登录后返回的，客户端需要存储,每次调用接口需要使用此参数发到服务器  
...
>  
	...
	videourl	视频地址 【调用2.68 上传视频接口】得到
	message		视频正文内容
	picids		图片id, 多个用|连接(如：1|2|3)，调用2.21单张图片上传接口得到picid
	friend		查看范围：1家人可见，2仅自己可见，0全站用户可见
	tags		空间名
...
> 
	...
	获取验证码：
	op:				 getseccode
	username		 绑定的手机号(该绑定是绑定到原来接口中username字段），一个号码只能绑定到一个用户。
	phonesubmit  	 1
	m_auth:		  	 API密钥, 由登录后返回的，客户端需要存储,每次调用接口需要使用此参数发到服务器  
...
<h3>2.54、	</h3><h3>订阅接口</h3>  
调用   2.7、		申请成为家人接口  applyuid即为被订阅的uid。
...
> 
	...
	date			 日期
	all				 若设为1则代表查看全部，默认为空只取自己的
	repeat			 若设为1则代表把按天，周，月重复的也取出来
	m_auth:		  	 API密钥, 由登录后返回的，客户端需要存储,每次调用接口需要使用此参数发到服务器  
...
> 
	...
	day			 	 某天【可选】，每月一次时才需要，（0-31）
	friends			 @的家人的uid【可选】，多个用|连接	可以调用2.33、和谁在一起的人列表接口 显示列表
	calendarsubmit   如果不设置，就是发布界面；如果设为1，则是保存
	m_auth:		  	 API密钥, 由登录后返回的，客户端需要存储,每次调用接口需要使用此参数发到服务器  
	
【调用方式】  
网站域名/dapi/cp.php?ac=calendar&recalendar    
【返回值】  
>  
...
<h3>2.68、	</h3><h3>上传视频接口</h3>  
【参数】  
> 

	videodata:		 上传声音文件的表单【FILE类型】 
	videotime:		 上传声音文件的时间长度 
	op		 		 uploadvideo 
	m_auth:		  	 API密钥, 由登录后返回的，客户端需要存储, 每次调用接口需要使用此参数发到服务器  
